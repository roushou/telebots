//! Image generation: request plumbing and response decoding.
...
use crate::{
    ...
    error::Error,
    types::{GeneratedImage, ImageInput, ImageModel},
};
...
impl CloudflareAiClient {
    ...
    pub async fn generate_image(
        ...
        let resp = match model.input() {
            ImageInput::Json => request.json(&json!({ "prompt": prompt })).send().await?,
        ...
        Self::decode_image(&content_type, &bytes)
    }
    ...
    fn decode_image(content_type: &str, bytes: &[u8]) -> Result<GeneratedImage, Error> {
...
mod tests {
    ...
    fn decode(content_type: &str, body: &[u8]) -> Result<GeneratedImage, Error> {
        CloudflareAiClient::decode_image(content_type, body)
    }
