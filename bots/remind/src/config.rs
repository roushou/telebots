...
/// Default database path used when `REMIND_DB_PATH` is unset.
pub const DEFAULT_DB_PATH: &str = "remind.db";
...
pub struct Config {
    ...
    /// SQLite database path (`REMIND_DB_PATH`).
    #[serde(rename = "remind_db_path", default = "default_db_path")]
    pub db_path: String,
...
mod tests {
    ...
    fn debug_redacts_secrets() {
        ...
        assert!(debug.contains("remind.db"));
    }
    ...
    fn loads_from_environment() -> Result<()> {
        ...
        assert_eq!(cfg.db_path, "/data/remind.db");
        assert_eq!(cfg.metrics_port, 9104);
        Ok(())
    }

    #[test]
    fn defaults_apply_when_unset() -> Result<()> {
        let cfg = Config::from_source(env(&[("TELEBOTS_TELEGRAM_API_KEY", "tok")]))?;
